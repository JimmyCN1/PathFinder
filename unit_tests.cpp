...
class Data {
public:
  Data() : maze(NULL), rows(0), cols(0), initial(), goal(), m3(false){};
  ...
  if (ok) {
    ...
    if (DEBUG) {
      std::cout << "Loaded Initial Position: "
                ...
                << std::get<TUPLE_Y>(data->initial) << ")" << std::endl;
    }
  } else {
...
bool run_unit_test(DataPtr data) {
  ...
  if (numPositions > 0) {
    if (DEBUG) {
      ...
      for (int i = 0; i != numPositions; ++i) {
        std::cout << "(" << finalPositions->get(i)->getX() << ","
                  << finalPositions->get(i)->getY() << ")" << std::endl;
      }
    }
  ...
  if (data->m3) {
    ...
    if (DEBUG) {
      ...
      for (int i = 0; i != numPositions; ++i) {
        std::cout << "(" << path->get(i)->getX() << "," << path->get(i)->getY()
                  << ")" << std::endl;
      }
    }
